...
// ─── Single stat row ──────────────────────────────────────────────────────────
function StatRow({ label, v1, v2, higherIsBetter = true, formatter = (x) => x }) {
...
function DiffBar({ label, v1, v2, color }) {
  const max   = Math.max(v1, v2, 1);
  const pct1  = Math.round((v1 / max) * 100);
  const pct2  = Math.round((v2 / max) * 100);
  return (
    <div className="cmp-bar-row">
      <span className="cmp-bar-val cmp-bar-val--left">{v1}</span>
      <div className="cmp-bar-track">
        {/* left bar grows from centre leftward */}
        <div className="cmp-bar-fill cmp-bar-fill--left"  style={{ width: `${pct1}%`, background: color }} />
        <span className="cmp-bar-label">{label}</span>
        <div className="cmp-bar-fill cmp-bar-fill--right" style={{ width: `${pct2}%`, background: color }} />
      </div>
      <span className="cmp-bar-val cmp-bar-val--right">{v2}</span>
    </div>
  );
}

// ─── Player header card ───────────────────────────────────────────────────────
function PlayerCard({ username, isWinner }) {
  const initials = username.slice(0, 2).toUpperCase();
  return (
    <div className={`cmp-player-card ${isWinner ? 'cmp-player-card--winner' : ''}`}>
...
export default function ComparePage({ darkMode }) {
  ...
  if (data1 && data2) {
    const score1 = (data1.solved.totalSolved || 0) * 3
                 + (data1.contest?.contestRating || 0) * 0.5;
    const score2 = (data2.solved.totalSolved || 0) * 3
                 + (data2.contest?.contestRating || 0) * 0.5;
  ...
  return (
    <div className="container mt-4 pb-5">
      ...
      <div className="text-center mb-4">
        ...
        <p className="text-muted mb-0">See how two LeetCoders stack up head-to-head</p>
      </div>

      {/* ── Input card ── */}
      <div className="card shadow-sm p-4 mb-4 cmp-input-card">
        <div className="row g-3 align-items-end">
          <div className="col-md-5">
            <label className="form-label small fw-semibold text-muted">Player 1</label>
            <input
              type="text"
          ...
          <div className="col-md-5">
            <label className="form-label small fw-semibold text-muted">Player 2</label>
            <input
              type="text"
              className="form-control"
      ...
      {/* ── Results ── */}
      {data1 && data2 && (
        <div className="cmp-results-card card shadow-sm p-4">

          {/* Overall winner banner */}
          {overallWinner !== 0 && (
            <div className="cmp-winner-banner mb-4">
              🏆 <strong>{overallWinner === 1 ? data1.username : data2.username}</strong> is ahead overall!
            </div>
          )}
          {overallWinner === 0 && (
            <div className="cmp-winner-banner cmp-winner-banner--tie mb-4">
          ...
          <div className="row g-3 mb-4">
            <div className="col-6">
              <PlayerCard username={data1.username} isWinner={overallWinner === 1} />
            </div>
            <div className="col-6">
              <PlayerCard username={data2.username} isWinner={overallWinner === 2} />
            </div>
          </div>
          ...
          <div className="cmp-bars-section mb-2">
            <h6 className="text-muted text-uppercase small fw-bold mb-3 text-center">
